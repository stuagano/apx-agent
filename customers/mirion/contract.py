"""Frozen pre-call brief contract for Mirion — the single source of truth.
...
Swapping synthetic->real later repoints each ``sql/vw_*.sql`` at real tables;
these column lists (and therefore the agent grounding) do not change.
"""
...
# Brief sections in render order (title -> view). Mirrors customers/mirion.toml
...
# names only (no real customers); swap in your own seed set locally. The three
# archetypes mirror Mirion's segments (nuclear, medical, lab) without naming any
# real facility. No dependency on any real system.
