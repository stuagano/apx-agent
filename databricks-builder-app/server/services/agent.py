...
async def stream_agent_response(
  ...
  try:
    ...
    # Connect to Databricks MCP server via SSE, then filter tools based on enabled skills
    ...
    if blocked_count:
      logger.info(f'Databricks MCP server (SSE): {len(filtered_tool_names)} tools allowed, {blocked_count} blocked by disabled skills')
    else:
      logger.info(f'Databricks MCP server (SSE) configured with {len(filtered_tool_names)} tools')

    # Get apx tools (workspace file upload + app create/deploy/status)
    apx_server, apx_tool_names = load_apx_tools()
    allowed_tools.extend(apx_tool_names)
    logger.info(f'apx MCP server configured with {len(apx_tool_names)} tools')
