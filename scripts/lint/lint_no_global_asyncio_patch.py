...
When test code writes::
    ...
    with patch("apx_agent.tools.mcp.asyncio.sleep", new_callable=AsyncMock):
        ...

``unittest.mock``'s dotted-path resolver walks:

1. ``import apx_agent.tools.mcp``
2. ``getattr(apx_agent.tools.mcp, "asyncio")`` -> returns the
...
top of it) silently get the mock.
    ...
    for arg in argv[1:]:
        ...
        if hits:
            ...
            for line, snippet in hits:
                ...
                sys.stdout.write(
                    "   hint: see scripts/lint/lint_no_global_asyncio_patch.py "
