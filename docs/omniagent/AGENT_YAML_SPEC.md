...
OmniAgents can run an agent from a single YAML file:

```bash
omniagents run path/to/agent.yaml --profile <databricks-profile>
```
...
CLI flags such as `--profile`, `--harness`, and `--model` can override or supply
missing executor values for a run.
...
- Run the YAML before publishing it:
  ...
  ```bash
  omniagents run path/to/agent.yaml --profile <profile> -p "Say hello"
  ```
