...
policies:
  session_budget:
    type: function
    handler: omniagents.policies.builtins.cost.cost_budget
    factory_params:
      max_cost_usd: 10.00
      ask_thresholds_usd: [5.00]
  global_rate_limit:
    type: function
    handler: omniagents.policies.builtins.safety.max_tool_calls_per_session
    factory_params:
      limit: 200
```
...
```bash
omniagents server --config server_config.yaml
```
...
policies:
  ...
  github_access:
    type: function
    handler: omniagents.policies.builtins.github.github_policy
    factory_params:
      write_repos:
        - myorg/my-repo
      write_branches:
  ...
  google_policy:
    type: function
    handler: omniagents.policies.builtins.google.gdrive_policy
    factory_params:
      read_all: true
      allow_create: true
```
...
approve_file_ops:
  type: function
  handler: omniagents.policies.builtins.safety.ask_on_os_tools
```
...
Session-level policies let you customize agent behavior for your current task. In the OmniAgents UI, open the information window to browse available policies and toggle them on or off for your session.
...
no_deploy_skill:
  type: function
  handler: omniagents.policies.builtins.safety.block_skills
  factory_params:
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `sandbox_type` | string | `"linux_bwrap"` | Sandbox backend (`linux_bwrap`, `darwin_seatbelt`, `linux_landlock`, `none`) |
...
Gates a session on cumulative LLM spend. ASKs at soft warning thresholds, DENYs at the hard limit.

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `max_cost_usd` | number | (required) | Hard spend limit in USD |
| `ask_thresholds_usd` | number[] | `null` | Soft warning checkpoints (each must be < `max_cost_usd`) |
...
policies:
  deny_trivial_opus:
    type: function
    handler: omniagents.policies.builtins.routing.deny_trivial_to_expensive_model
    factory_params:
      expensive_models:
...
```python
from omniagents.policies.schema import PolicyEvent, PolicyResponse
