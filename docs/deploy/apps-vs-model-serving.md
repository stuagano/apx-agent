...
| **Async background work** | No (request-bound) | Yes — background tasks, schedulers, and queues are possible; durable resume still requires persisted state and an explicit task/continuation protocol |
...
- **Horizontal scaling requires durable session state.** Apps can run 1–5
  ...
  do not rely on instance-local state. Declare the replica count once:
