...
databricks current-user me --profile <name>   # must succeed before apx run
...
- **Databricks Apps must be enabled** in your workspace. Check under **Workspace Settings → Apps**. Without this, `apx deploy` will fail. If Apps isn't available, ask your workspace admin or use `--target model-serving` instead.
...
From inside `my-agent/`, this bundles the project and creates a Databricks App. `apx deploy` prints the app URL when it finishes.

(For Model Serving instead: `apx deploy --target model-serving --name <catalog.schema.model>`.)
...
After the initial deploy, `apx deploy` uploads your code to a path in your Databricks workspace. To update without redeploying from the CLI:
...
**`apx deploy` fails with Apps error** — confirm Apps is enabled in your workspace under **Workspace Settings → Apps**. If not available, use `--target model-serving` instead.
