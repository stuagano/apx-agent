...
- **`fleet redeploy` v1 mechanism:** for each selected agent, find the latest registered version and point the `@prod` alias at it (`_apps_registry.set_prod_alias_version`) if it differs from the current alias (`_apps_registry.get_prod_alias_version`). No git rebuild, no serving-config patch.
  - **ERRATA (applied during implementation, see commit `80ba1cfc`):** Task 8 below references `get_latest_apps_version`, which maxes over **all** roles including canary — promoting an un-soaked canary to `@prod` mid-soak (the hazard PR #176 fixed). The shipped code and tests use **`get_latest_prod_version`** instead (prod manifests only; non-prod/non-apps agents skipped). Read the Task 8 code blocks with that substitution.
