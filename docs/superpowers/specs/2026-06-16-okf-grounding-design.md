...
### Reverse (OKF → col-string; `_load_okf_schema`)

- Pipe row `| \`gross_pay\` | decimal(6,2) | ... |` → strip backticks from col, take Type cell **verbatim** → clean concat `gross_pay(decimal(6,2))`. (The only nested-paren subtlety is on the **forward** parse, not here.)
- Bullet row `- \`event_date\` (STRING): ...` → `event_date(STRING)` (best-effort forward-compat for ga4-style bodies).
