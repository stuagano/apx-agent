"""samples-tpcds-sf1000 — apx-agent root agent (ADK-style top-level definition).

Generated by ``apx-agent agents scaffold samples-tpcds-sf1000 --target apps``.
...
def sample_call_center(ws: Dependencies.Workspace) -> dict:
    """Preview a few rows of `samples.tpcds_sf1000.call_center` — your first tool.
    ...
    """
    return run_sql(ws, "SELECT * FROM `samples`.`tpcds_sf1000`.`call_center` LIMIT 5")



# A governed data agent over ``samples.tpcds_sf1000`` (schema baked at scaffold
...
_CATALOG = "samples"
_SCHEMA = "tpcds_sf1000"

agent = DataAgent(
    os.environ.get("APX_CATALOG", _CATALOG),
    os.environ.get("APX_SCHEMA", _SCHEMA), extra_tools=[sample_call_center],
    name="samples-tpcds-sf1000", knowledge="./.apx/okf",
)
