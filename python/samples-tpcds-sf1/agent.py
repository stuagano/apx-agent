"""samples-tpcds-sf1 — apx-agent root agent (ADK-style top-level definition).

Generated by ``apx-agent agents scaffold samples-tpcds-sf1 --target apps``.
...
def sample_call_center(ws: Dependencies.Workspace) -> dict:
    """Preview a few rows of `samples.tpcds_sf1.call_center` — your first tool.
    ...
    """
    return run_sql(ws, "SELECT * FROM `samples`.`tpcds_sf1`.`call_center` LIMIT 5")



# A governed data agent over ``samples.tpcds_sf1`` (schema baked at scaffold
...
agent = DataAgent(
    ...
    name="samples-tpcds-sf1", knowledge="./.apx/okf",
)
