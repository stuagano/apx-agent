...
LakebaseExampleStore.find_similar/.list wrapped their query in
`except Exception: return []`. So a transient outage, an expired OAuth token,
or a missing GRANT made the store silently report "no examples" — prompt
