"""Claim-vs-reality: a configured governance that errors must fail CLOSED (#371).

`GovernanceClient.evaluate` returned `action="allow"` on any transport failure —
...
from apx_agent._governance import GovernanceClient, _run_coro_blocking
...
def test_transport_error_fails_closed_by_default() -> None:
    client = GovernanceClient(transport=_boom)
    decision = client.evaluate(operation="tool_call", context={})
    assert decision.action == "reject", "a configured governance that errors must fail closed"


@pytest.mark.unit
def test_fail_closed_false_restores_allow() -> None:
    client = GovernanceClient(transport=_boom, fail_closed=False)
    decision = client.evaluate(operation="tool_call", context={})
...
def test_non_decision_response_fails_closed() -> None:
    ...
    client = GovernanceClient(transport=lambda _req: "not a dict")
    assert client.evaluate(operation="tool_call", context={}).action == "reject"


@pytest.mark.unit
def test_noop_transport_still_allows() -> None:
    # No configured governance (default noop) must NOT start rejecting traffic.
    assert GovernanceClient().evaluate(operation="tool_call", context={}).action == "allow"
...
def test_run_coro_blocking_works_inside_running_loop() -> None:
    ...
    # governance outage. The helper must run the coroutine off the active loop.
