...
The `app.yml` file at the root of this example defines the startup command and environment variables. `databricks apps deploy` (or `uv run apx-agent agents deploy`) uploads the source tree and starts the app.
...
```bash
uv run apx-agent agents deploy           # package + deploy to Databricks Apps
```

The `apx-agent` CLI handles packaging the app as a Python wheel, copying static assets, and calling `databricks apps deploy`. During local development, `uv run uvicorn ... --reload` works without any apx-agent-specific tooling.
...
`apx-agent agents deploy` (and `databricks bundle deploy`) reads this file to run the build and deploy the result. You rarely edit this unless you're changing the bundle name or adding new resources (jobs, pipelines, etc.).
