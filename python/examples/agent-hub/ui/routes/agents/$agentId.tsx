      ...
      <div className="rounded-xl border bg-card p-5">
        ...
        {response !== null && (
          <div className="p-4 rounded-lg bg-muted/50 text-sm whitespace-pre-wrap font-mono leading-relaxed max-h-96 overflow-y-auto">
            {response || <span className="animate-pulse text-muted-foreground">Thinking…</span>}
          </div>
        )}
      </div>
    </section>
              ...
              {agent.tools.length === 0 ? (
                ...
                <div className="grid gap-2">
                  {agent.tools.map((tool) => (
                    ...
                    >
                      <code className="shrink-0 px-2 py-0.5 rounded bg-muted text-xs font-mono">{tool.name}</code>
