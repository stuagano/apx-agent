...
def create_investigation_pipeline(data_inspector_url: str) -> "_DeterministicRouter":
...
class _DeterministicRouter(BaseAgent):
    ...
    Investigation keywords → full pipeline. Everything else → general agent.

    Exposes ``.investigation`` and ``.general`` as public attributes so the
    Apps target can compile each branch separately via
    ``compile_to_responses_agent`` (which doesn't recognize the router
    subclass directly) and do the keyword routing at the request layer.
    """
    ...
    def matches_investigation(text: str) -> bool:
        ...
        string so callers don't need to construct apx-agent ``Message``
        objects.
        """
        lower = text.lower()
