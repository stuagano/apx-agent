...
def _run_checked(argv: list[str], *, cwd: Path) -> str:
    """Run *argv* in *cwd*; raise ClickException on non-zero exit."""
    proc = subprocess.run(
        argv, cwd=str(cwd), capture_output=True, text=True, check=False,
    )
    if proc.returncode != 0:
        detail = (proc.stderr or proc.stdout or "").strip() or f"exit {proc.returncode}"
        raise click.ClickException(f"{' '.join(argv)} failed: {detail}")
    return (proc.stdout or "").strip()


def _git_repo_root(cwd: Path) -> Path:
    """Return the git toplevel that contains *cwd*."""
    return Path(_run_checked(["git", "rev-parse", "--show-toplevel"], cwd=cwd))


def _write_push_drift_report(
    okf_root: Path,
    *,
    catalog: str,
    schema: str,
    ...
    """Write a reviewable OKF→UC drift report under the OKF bundle."""
    path = okf_root / "DRIFT.md"
    path.write_text(
        f"# OKF → UC comment drift\n\n"
        f"Source of truth: local OKF (`.apx/okf`).\n"
        f"Target: `{catalog}.{schema}`.\n\n"
        f"Generated by `apx-agent agents drift-pr --direction push`.\n\n"
        f"## Plan\n\n```\n{plan}\n```\n\n"
        f"## Apply\n\n"
        f"After review, from the project root (does **not** run from this PR "
        f"alone — UC writes need a live warehouse):\n\n"
        f"```bash\n"
        f"uv run apx-agent agents push-comments --yes\n"
        f"```\n"
    )
    return path


def _commit_paths_and_open_pr(
    *,
    repo: Path,
    branch: str,
    title: str,
    ...
    """Commit *paths* on a new branch and open a GitHub PR via ``gh``.

    Returns the PR URL, or ``None`` on ``dry_run``. Paths must exist under
    *repo*. Only those paths are staged — unrelated dirty files are ignored.
    """
    rels: list[str] = []
    for p in paths:
        resolved = p.resolve()
        try:
            rels.append(str(resolved.relative_to(repo.resolve())))
        except ValueError as e:
            raise click.ClickException(
                f"path {p} is outside the git repo at {repo}"
            ) from e
    if dry_run:
        click.echo(f"drift-pr: dry-run — would branch {branch!r}")
        click.echo(f"drift-pr: dry-run — would commit: {', '.join(rels)}")
        click.echo(f"drift-pr: dry-run — would open PR: {title}")
        click.echo(body)
        return None

    _run_checked(["git", "checkout", "-b", branch], cwd=repo)
    _run_checked(["git", "add", "--", *rels], cwd=repo)
    staged = _run_checked(
        ["git", "diff", "--cached", "--name-only", "--", *rels], cwd=repo,
    )
    if not staged.strip():
        raise click.ClickException(
            "drift-pr: nothing staged after preparing changes — aborting."
        )
    _run_checked(["git", "commit", "-m", title], cwd=repo)
    _run_checked(["git", "push", "-u", "origin", "HEAD"], cwd=repo)
    url = _run_checked(
...
@agents.command("drift-pr")
@click.option(
    "--direction",
    type=click.Choice(["push", "pull"]),
    required=True,
    help=(
        "'push': OKF is source of truth — commit a DRIFT.md plan and open a PR "
        "proposing `push-comments --yes`. "
        "'pull': UC is source of truth — apply `pull-comments` into OKF files "
        "and open a PR with those edits."
    ),
)
@click.option(
    "--profile", default=None,
    help="Databricks CLI profile. Falls back to $DATABRICKS_CONFIG_PROFILE.",
)
@click.option(
    "--overwrite", is_flag=True, default=False,
    help="With --direction pull: replace curated OKF text with UC comments "
         "(same semantics as `pull-comments --overwrite`).",
)
@click.option(
    "--dry-run", is_flag=True, default=False,
    help="Compute drift and print the PR plan without git/gh side effects. "
         "For pull, temporarily applies then restores OKF table files.",
)
def drift_pr(
    direction: str,
    profile: str | None,
    ...
    """Open a PR when OKF comments drift from live Unity Catalog.

    Scheduled form of the pull/push comment loop: detect drift, commit a
    reviewable change set, and open a GitHub PR. Never writes COMMENT ON to UC
    from this command — ``--direction push`` only proposes ``push-comments``.

    Requires ``git`` + ``gh`` on PATH (unless ``--dry-run``), and a workspace
    connection that can list table comments.
    """
    from datetime import datetime, timezone
    ...
    repo = _git_repo_root(Path.cwd())

    if direction == "push":
        desired = okf_comments_for_uc(okf_root)
        changes = diff_okf_uc_comments(desired, live)
        plan = format_comment_diff(changes)
        click.echo(plan)
        if not changes:
            click.echo(f"drift-pr: {catalog}.{schema} already matches OKF — no PR.")
            return
        report = _write_push_drift_report(
            okf_root, catalog=catalog, schema=schema, plan=plan,
        )
        branch = f"okf-drift/push-{stamp}"
        title = f"okf: propose push-comments for {catalog}.{schema} ({len(changes)} change(s))"
        body = (
            f"## Summary\n"
            f"- Detected {len(changes)} OKF→UC comment drift(s) in "
            f"`{catalog}.{schema}`.\n"
            f"- Added `.apx/okf/DRIFT.md` with the plan.\n"
            f"- After merge/review, apply with "
            f"`uv run apx-agent agents push-comments --yes` "
            f"(this PR does not write to UC).\n"
        )
        try:
            url = _commit_paths_and_open_pr(
                repo=repo,
                branch=branch,
                title=title,
                body=body,
    ...
    n = apply_uc_comments(okf_root, live, overwrite=overwrite)
    changed_paths = [
        p for p, before in snapshots.items()
        if p.is_file() and p.read_text() != before
    ]
    # New files shouldn't appear from apply_uc_comments (only existing tables).
    if dry_run:
        for p, before in snapshots.items():
            p.write_text(before)
        if n == 0 or not changed_paths:
            click.echo(
                f"drift-pr: dry-run — no OKF enrichment from {catalog}.{schema}."
            )
            return
        click.echo(
            f"drift-pr: dry-run — would update {len(changed_paths)} OKF table "
            f"file(s) from {catalog}.{schema} and open a PR."
        )
        for p in changed_paths:
            click.echo(f"  {p.relative_to(Path.cwd())}")
        return
    if n == 0 or not changed_paths:
        click.echo(
            f"drift-pr: OKF already reflects UC comments for {catalog}.{schema} "
            f"— no PR."
        )
        return
    branch = f"okf-drift/pull-{stamp}"
    title = (
        f"okf: pull UC comments into OKF for {catalog}.{schema} "
        f"({len(changed_paths)} table(s))"
    )
    body = (
        f"## Summary\n"
        f"- Pulled live Unity Catalog comments for `{catalog}.{schema}` into "
        f"`.apx/okf` ({len(changed_paths)} table file(s)).\n"
        f"- overwrite={'yes' if overwrite else 'no (empty cells only)'}.\n"
        f"- Review the OKF diffs; merge to adopt UC as the local source of truth.\n"
    )
    url = _commit_paths_and_open_pr(
        repo=repo,
        branch=branch,
