"""samples-tpch — apx-agent root agent (ADK-style top-level definition).

Generated by ``apx-agent agents scaffold samples-tpch --target apps``.
...
def sample_customer(ws: Dependencies.Workspace) -> dict:
    """Preview a few rows of `samples.tpch.customer` — your first tool.
    ...
    """
    return run_sql(ws, "SELECT * FROM `samples`.`tpch`.`customer` LIMIT 5")



# A governed data agent over ``samples.tpch`` (schema baked at scaffold
...
agent = DataAgent(
    ...
    os.environ.get("APX_SCHEMA", _SCHEMA), extra_tools=[sample_customer],
    name="samples-tpch", knowledge="./.apx/okf",
)
