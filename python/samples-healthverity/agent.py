"""samples-healthverity — apx-agent root agent (ADK-style top-level definition).

Generated by ``apx-agent agents scaffold samples-healthverity --target apps``.
...
def sample_claims_sample_synthetic(ws: Dependencies.Workspace) -> dict:
    """Preview a few rows of `samples.healthverity.claims_sample_synthetic` — your first tool.
    ...
    """
    return run_sql(ws, "SELECT * FROM `samples`.`healthverity`.`claims_sample_synthetic` LIMIT 5")



# A governed data agent over ``samples.healthverity`` (schema baked at scaffold
...
_CATALOG = "samples"
_SCHEMA = "healthverity"

agent = DataAgent(
    os.environ.get("APX_CATALOG", _CATALOG),
    os.environ.get("APX_SCHEMA", _SCHEMA), extra_tools=[sample_claims_sample_synthetic],
    name="samples-healthverity", knowledge="./.apx/okf",
)
