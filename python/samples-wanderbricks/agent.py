"""samples-wanderbricks — apx-agent root agent (ADK-style top-level definition).

Generated by ``apx-agent agents scaffold samples-wanderbricks --target apps``.
...
def sample_amenities(ws: Dependencies.Workspace) -> dict:
    """Preview a few rows of `samples.wanderbricks.amenities` — your first tool.
    ...
    """
    return run_sql(ws, "SELECT * FROM `samples`.`wanderbricks`.`amenities` LIMIT 5")



# A governed data agent over ``samples.wanderbricks`` (schema baked at scaffold
...
_CATALOG = "samples"
_SCHEMA = "wanderbricks"

agent = DataAgent(
    os.environ.get("APX_CATALOG", _CATALOG),
    os.environ.get("APX_SCHEMA", _SCHEMA), extra_tools=[sample_amenities],
    name="samples-wanderbricks", knowledge="./.apx/okf",
)
