"""samples-accuweather — apx-agent root agent (ADK-style top-level definition).

Generated by ``apx-agent agents scaffold samples-accuweather --target apps``.
...
def sample_forecast_daily_calendar_imperial(ws: Dependencies.Workspace) -> dict:
    """Preview a few rows of `samples.accuweather.forecast_daily_calendar_imperial` — your first tool.
    ...
    """
    return run_sql(ws, "SELECT * FROM `samples`.`accuweather`.`forecast_daily_calendar_imperial` LIMIT 5")



# A governed data agent over ``samples.accuweather`` (schema baked at scaffold
...
agent = DataAgent(
    ...
    os.environ.get("APX_SCHEMA", _SCHEMA), extra_tools=[sample_forecast_daily_calendar_imperial],
    name="samples-accuweather", knowledge="./.apx/okf",
)
